...
class monitoring():
	class receiving_data_from_arduino():
		...
		def grab_raw_input_from_arduino():
			print("yes")
		def convert_raw_input_into_data():
			print("converted")
	class graphs():
		...
		def animation_frame_pressure(i):
			...
			if animation_loop_counter_pressure<data_points_pressure:
				...
				y_data_pressure=np.append(y_data_pressure,[value_pressure])
			if animation_loop_counter_pressure>=data_points_pressure:
				y_data_pressure=np.append(y_data_pressure,[value_pressure])
		...
		def animation_frame_flow(i):
			...
			if animation_loop_counter_flow<data_points_flow:
				...
				y_data_flow=np.append(y_data_flow,[value_flow])
			if animation_loop_counter_flow>=data_points_flow:
				y_data_flow=np.append(y_data_flow,[value_flow])
		...
		def show_graphs():
			...
			value_pressure=5
			monitoring.graphs.canvas_pressure.place(x=0,y=0)
			monitoring.graphs.canvas_flow.place(x=0,y=500)
...
quit_button=Button(window, image=quit_button_image, highlightthickness=0,bd=0,bg="white",command=monitoring.graphs.show_graphs)
quit_button.place(x=799,y=50)

quit2_button_image=PhotoImage(file="cross.gif")
quit2_button=Button(window, image=quit2_button_image, highlightthickness=0,bd=0,bg="white",command=monitoring.graphs.hide_graphs)
