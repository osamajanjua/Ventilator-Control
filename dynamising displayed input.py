...
window = Tk()
window.title("Ventilator with class")
...
class monitoring():
    class receiving_data_from_arduino():
        print("nothing")
        def capture_raw_input_from_arduino():
            print("nothing")
    ...
    class graphs():
        ...
        global data_points_displayed
        global time_interval_millis
        ...
        input_data_array_flow=np.array([-12])
        x_axis_scale_flow = np.array([0])
        horizontal_axis_flow=np.array([0])
        global loop_counter_flow_graph
        global seconds_displayed_flow
        global time_interval_flow
        global data_points_displayed_flow
        global time_interval_millis_flow
...
#     monitoring.graphs.draw_flow_graph(12)
#     #monitoring.graphs.draw_pressure_graph(1)
#     if hide_the_graphs=="no":
#         # time_interval_millis
#         window.after(100, call_graph)
# a=0
def call_graph():
    global hide_the_graphs
    global a
    # if a!=0:
    #     monitoring.graphs.hide_all_graphs()
    #     a=1
    monitoring.graphs.draw_flow_graph(12)
    monitoring.graphs.draw_pressure_graph(1)
