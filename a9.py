...
class monitoring():
    ...
    class display_values_being_monitored():
        ...
        def display_insp_time(insp_time,max,min):
            ...
            line=line_number-1
            x_increment=0; y_increment=65
            insp_time_number=Label(window, text=insp_time, anchor="e", bg="black", fg="white",font=("montserrat",34,"normal"))
    ...
    class graphs():
        ...
        time_interval_pressure=0.1/data_points_in_one_array_pressure
        ...
        time_interval_flow=0.1/data_points_in_one_array_flow
        total_time_flow=5
    ...
    def live_updates():
        ...
        if testing_time_start<testing_time_end:
            window.after(100,monitoring.live_updates)
...
def send_data_to_arduino():
    global packed_manoeuvre
    if trigger_mode=="P":
        str_pack=str(len(packed_pressure))+"s"+str(len(packed_peep))+"s"+str(len(packed_oxygen))+"s"+str(len(packed_bpm))+"s"+str(len(packed_vol))+"s"+str(len(packed_insp_time))+"s"+str(len(packed_triggering_pressure))+"s"+str(len(packed_ie))+"s"+str(len(packed_manoeuvre))+"s"
        string_to_send=struct.pack(str_pack,packed_pressure.encode('UTF-8'),packed_peep.encode('UTF-8'),packed_oxygen.encode('UTF-8'),packed_bpm.encode('UTF-8'),packed_vol.encode('UTF-8'),packed_insp_time.encode('UTF-8'),packed_triggering_pressure.encode('UTF-8'),packed_ie.encode('UTF-8'),packed_manoeuvre.encode('UTF-8'))
        arduino_data.write(string_to_send)
    if trigger_mode=="F":
        str_pack=str(len(packed_pressure))+"s"+str(len(packed_peep))+"s"+str(len(packed_oxygen))+"s"+str(len(packed_bpm))+"s"+str(len(packed_vol))+"s"+str(len(packed_insp_time))+"s"+str(len(packed_triggering_flow))+"s"+str(len(packed_ie))+"s"+str(len(packed_manoeuvre))+"s"
        string_to_send=struct.pack(str_pack,packed_pressure.encode('UTF-8'),packed_peep.encode('UTF-8'),packed_oxygen.encode('UTF-8'),packed_bpm.encode('UTF-8'),packed_vol.encode('UTF-8'),packed_insp_time.encode('UTF-8'),packed_triggering_flow.encode('UTF-8'),packed_ie.encode('UTF-8'),packed_manoeuvre.encode('UTF-8'))
        arduino_data.write(string_to_send)
    packed_manoeuvre="m,6,"
