...
class monitoring():
    ...
    class testing_inputs_and_playing_alarms():
        ...
        def testing_received_values():
            ...
            if value_pressure_received_5<=LL_pressure:
                notification_label_pressure.configure(text="Pressure kam ee")
            if value_pressure_received_5>=UL_pressure:
                notification_label_pressure.configure(text="Pressure baun zada ee")
            if value_pressure_received_5>LL_pressure and value_pressure_received_5<UL_pressure:
                notification_label_pressure.configure(text="Pressure clear ae")

            if value_peep_received_5<=LL_peep:
                notification_label_peep.configure(text="peep kam ee")
            if value_peep_received_5>=UL_peep:
                notification_label_peep.configure(text="peep baun zada ee")
            if value_peep_received_5>LL_peep and value_peep_received_5<UL_peep:
                notification_label_peep.configure(text="peep clear")

            if value_pplat_received_5<=LL_pplat:
                notification_label_pplat.configure(text="pplat kam ee")
            if value_pplat_received_5>=UL_pplat:
                notification_label_pplat.configure(text="pplat baun zada ee")
            if value_pplat_received_5>LL_pplat and value_pplat_received_5<UL_pplat:
                notification_label_pplat.configure(text="pplat clear")

            if value_bpm_received_5<=LL_bpm:
                notification_label_bpm.configure(text="bpm kam ee")
            if value_bpm_received_5>=UL_bpm:
                notification_label_bpm.configure(text="bpm baun zada ee")
            if value_bpm_received_5>LL_bpm and value_bpm_received_5<UL_bpm:
                notification_label_bpm.configure(text="bpm clear")

            if value_oxygen_received_5<=LL_oxygen:
                notification_label_oxygen.configure(text="oxygen kam ee")
            if value_oxygen_received_5>=UL_oxygen:
                notification_label_oxygen.configure(text="oxygen baun zada ee")
            if value_oxygen_received_5>LL_oxygen and value_oxygen_received_5<UL_oxygen:
                notification_label_oxygen.configure(text="oxygen clear")
    ...
    class receiving_data_from_arduino():
        ...
        def grab_raw_input_from_arduino():
            ...
            global i
            if i==0:
                data_received="PP,10,10,10,10,10,PEEP,31,31,31,31,111,PPl,32,32,32,32,32,bpm,16,16,16,16,16,IE,12,12,12,12,12,FiO2,50,50,50,50,1,"
            if i==1:
                data_received="PP,0,0,0,0,0,PEEP,31,31,31,31,111,PPl,32,32,32,32,32,bpm,16,16,16,16,16,IE,12,12,12,12,12,FiO2,50,50,50,50,1,"
                i=-1
            i=i+1
