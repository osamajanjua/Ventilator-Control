...
class monitoring():
    ...
    class receiving_data_from_arduino():
        ...
        def grab_raw_input_from_arduino():
            ...
            # data_received="PP,150,150,150,150,150,PEEP,31,31,31,31,111,PPl,32,32,32,32,32,bpm,16,16,16,16,16,IE,12,12,12,12,12,FiO2,50,50,50,50,1,V,2000,2000,2000,2000,2000,"
        ...
        def convert_raw_input_into_values():
            ...
            global value_vol_received_1
            global value_vol_received_2
            global value_vol_received_3
            global value_vol_received_4
            global value_vol_received_5
            global opened_string
            opened_string=data_received.split(',')
            if len(opened_string)>=7:
                value_pressure_received_1=int(float(opened_string[1])*0.0102)
                value_pressure_received_2=int(float(opened_string[2])*0.0102)
                value_pressure_received_3=int(float(opened_string[3])*0.0102)
                value_pressure_received_4=int(float(opened_string[4])*0.0102)
                value_pressure_received_5=int(float(opened_string[5])*0.0102)
            if len(opened_string)>7:
                value_peep_received=int(opened_string[7])
                value_pplat_received=int(opened_string[9])
                value_bpm_received=int(opened_string[11])
                value_IE_received=int(opened_string[13])
                value_oxygen_received=int(opened_string[15])
                value_vol_received_1=int(opened_string[17])
                value_vol_received_2=int(opened_string[18])
                value_vol_received_3=int(opened_string[19])
                value_vol_received_4=int(opened_string[20])
                value_vol_received_5=int(opened_string[21])
    ...
    class display_values_being_monitored():
        ...
        def configure_monitored_values():
            ...
            global FiO2_number
            global insp_time_number
            Peak_P_number.configure(text=value_pressure_received_5)
            PEEP_number.configure(text=value_peep_received)
            P_plat_number.configure(text=value_pplat_received)
            BPM_number.configure(text=value_bpm_received)
            VTE_number.configure(text=value_vol_received_5)
            IE_number.configure(text=value_IE_received)
        ...
        def display_monitored_values():
            ...
            monitoring.display_values_being_monitored.display_VTE(value_vol_received_5,1400,500)
            monitoring.display_values_being_monitored.display_IE(value_IE_received,1.5,1.25)
    ...
    class graphs():
        ...
        # animation_pressure = FuncAnimation(fig_pressure, func=animation_frame_pressure, frames=1000, interval=100)
        ...
        animation2 = FuncAnimation(fig_flow, func=animation_frame_flow, frames=1000, interval=100)
...
class ventilation_inputs():
    ...
    class CustomScale(ttk.Scale):
        ...
        def _update_text(self, *args):
            ...
            if property_being_updated=="ie":
                ...
                packed_ie="ie"+","+str(value_ie_entered)+","
    ...
    def create_labels_that_show_me_my_inputs():
        ...
        unit_displayed_value_pressure_entered=  Label(window, text="mmH20",   bg="black", fg="white",font=("montserrat light",9,"normal"))
        displayed_value_peep_entered=           Label(window, text=value_peep_entered,   bg="black", fg="white",font=("montserrat light",30,"normal"))
        unit_displayed_value_peep_entered=      Label(window, text="mmH20",   bg="black", fg="white",font=("montserrat light",9,"normal"))
        ...
        if trigger_mode=="P":
            ...
            unit_displayed_triggering_pressure_entered= Label(window, text="mmH20",   bg="black", fg="white",font=("montserrat light",9,"normal"))
        if trigger_mode=="F":
            ...
            unit_displayed_triggering_flow_entered= Label(window, text="mmH20",   bg="black", fg="white",font=("montserrat light",9,"normal"))
...
def send_data_to_arduino():
    if trigger_mode=="P":
        str_pack=str(len(packed_pressure))+"s"+str(len(packed_peep))+"s"+str(len(packed_oxygen))+"s"+str(len(packed_bpm))+"s"+str(len(packed_vol))+"s"+str(len(packed_insp_time))+"s"+str(len(packed_triggering_pressure))+"s"
        string_to_send=struct.pack(str_pack,packed_pressure.encode('UTF-8'),packed_peep.encode('UTF-8'),packed_oxygen.encode('UTF-8'),packed_bpm.encode('UTF-8'),packed_vol.encode('UTF-8'),packed_insp_time.encode('UTF-8'),packed_triggering_pressure.encode('UTF-8'))
        arduino_data.write(string_to_send)
    if trigger_mode=="F":
        str_pack=str(len(packed_pressure))+"s"+str(len(packed_peep))+"s"+str(len(packed_oxygen))+"s"+str(len(packed_bpm))+"s"+str(len(packed_vol))+"s"+str(len(packed_insp_time))+"s"+str(len(packed_triggering_flow))+"s"
        string_to_send=struct.pack(str_pack,packed_pressure.encode('UTF-8'),packed_peep.encode('UTF-8'),packed_oxygen.encode('UTF-8'),packed_bpm.encode('UTF-8'),packed_vol.encode('UTF-8'),packed_insp_time.encode('UTF-8'),packed_triggering_flow.encode('UTF-8'))
        arduino_data.write(string_to_send)
